...
def predict():
    ...
    future_dates = [stock_data.index[-1] + timedelta(days=i) for i in range(1, no_of_days + 1)]

    # Generate Plots
    fig1 = plt.figure(figsize=(15, 6))
    plt.plot(stock_data['Close'], 'b', label='Close Price')
    plt.axhline(y=stock_data['Close'].max().item(), color='red', linestyle='--', label='Max Price')
    ...
    plt.title("Predicted Prices for the Next Days")
